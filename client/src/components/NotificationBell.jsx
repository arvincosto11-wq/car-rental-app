...
const NotificationBell = ({ isDark, iconColor, btnBg, btnBorder }) => {
  ...
  const s = {
    ...
    panel: {
      ...
      overflowY: 'auto', background: isDark ? '#242526' : '#fff', border: `1px solid ${isDark ? '#3a3b3c' : '#e5e7eb'}`,
    ...
    itemTitle: { fontSize: '12px', fontWeight: '600', color: isDark ? '#e4e6eb' : '#1a1a1a', flex: 1 },
    itemMsg: { fontSize: '11px', color: isDark ? '#b0b3b8' : '#6b7280', marginTop: '2px' },
    itemTime: { fontSize: '10px', color: isDark ? '#8a8d91' : '#9ca3af', marginTop: '4px' },
    deleteBtn: {
      background: 'none', border: 'none', cursor: 'pointer', padding: '2px 4px', flexShrink: 0,
      color: isDark ? '#8a8d91' : '#9ca3af', fontSize: '14px', lineHeight: 1,
    },
